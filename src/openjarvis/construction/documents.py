    ...
    *,
    ...
    for raw in raw_items:
        if not isinstance(raw, dict):
            continue
        description = str(
            raw.get("description") or raw.get("catalog_item") or ""
        ).strip()
        if not description:
            continue
        try:
...
_BASE_CSS = """
  ...
  th {
    background: #f5f5f7; font-size: 13px; text-transform: uppercase;
    letter-spacing: .03em;
  }
  ...
  .sign div {
    border-top: 1px solid #1a1a1a; padding-top: 6px; width: 240px; font-size: 13px;
  }
...
<style>{_BASE_CSS}</style></head><body>
  ...
  <footer>Generated by OpenJarvis.
  Review with qualified counsel before signing.</footer>
</body></html>"""
