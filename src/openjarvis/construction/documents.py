...
_BASE_CSS = """
  ...
  .scope { margin-top: 28px; font-size: 14px; line-height: 1.5; }
  .scope h4 { margin: 18px 0 8px; font-size: 15px; }
  .scope ul { margin: 0 0 12px 20px; padding: 0; }
...
def format_scope_html(text: str) -> str:
    """Turn plain-text scope notes into safe HTML (bullets, sections, paragraphs)."""
    text = (text or "").strip()
    if not text:
        return ""
    ...
    def flush_bullets() -> None:
        nonlocal bullet_buf
        if bullet_buf:
            items = "".join(f"<li>{_e(b)}</li>" for b in bullet_buf)
    ...
    def flush_para() -> None:
        nonlocal para_buf
        if para_buf:
            blocks.append(f"<p>{'<br>'.join(_e(p) for p in para_buf)}</p>")
            para_buf = []
    ...
    for raw in text.splitlines():
        ...
        if line.endswith(":") and len(line) < 80:
            flush_bullets()
            flush_para()
            blocks.append(f"<h4>{_e(line[:-1])}</h4>")
            continue
        flush_bullets()
...
def render_quote_html(
    *,
    ...
    if scope_of_work:
        scope_html = (
            "<div class='scope'><h3>Scope of Work</h3>"
            f"{format_scope_html(scope_of_work)}</div>"
        )
    exclusions_html = ""
    if exclusions:
        exclusions_html = (
            "<div class='scope'><h3>Exclusions</h3>"
            f"{format_scope_html(exclusions)}</div>"
        )
    timeline_html = ""
    if estimated_timeline:
        timeline_html = (
            f"<div class='scope'><h3>Estimated Timeline</h3>"
            f"<p>{_e(estimated_timeline)}</p></div>"
        )
